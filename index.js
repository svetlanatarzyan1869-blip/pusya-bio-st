...
(async function () {
  ...
  function buildUserPrompt(cx, last, lock, dateHint, logged) {
    ...
    else if (lock && lock.kind === 'postpartum') parts.push('=== FIXED BY CALENDAR ===\n' + (lock.pp_loss ? 'This is recovery after a pregnancy LOSS (' + lock.pp_loss + ') — there is NO baby and no lactation; keep the flavor about bleeding, hormonal crash and grief, gentle and never blaming. Output "pp_loss: ' + lock.pp_loss + '".' : 'This is the POSTPARTUM period after a birth.') + ' pp_day is FIXED at ' + lock.pp_day + ' of about ' + lock.pp_total + ' days of bleeding. Output EXACTLY "pp_day: ' + lock.pp_day + '" and "pp_total: ' + lock.pp_total + '" — never change them. The cycle has NOT returned yet (it is expected around ' + lock.back + '); do NOT switch to the cycle/heat state on your own — the calendar does it. IMPORTANT: still fill ALL other flavor fields FULLY.');
    ...
    else if (lock && lock.first) parts.push('=== FIXED BY CALENDAR ===\nThe cycle has JUST RETURNED after a pregnancy: this is the FIRST cycle, day ' + lock.day + ' of ' + lock.length + '. Output EXACTLY "day: ' + lock.day + '" and "length: ' + lock.length + '". The first cycles after a pregnancy are often irregular and a bit longer — reflect that in the flavor. IMPORTANT: fill ALL flavor fields FULLY.');
  ...
  // когда вернётся цикл после родов/потери и какой длины будет первый цикл.
  // После потери месячные обычно через 4–6 нед, после родов — дольше (6–11 нед, с лактацией позже).
  function ppReturn(ap, seed, baseLen) {
    var birth = parseISO(ap.birth), k = seed + '|back|' + ap.birth;
    var days = ap.loss ? 28 + Math.floor(srand(k) * 15) : 45 + Math.floor(srand(k) * 31);
    var L = num(baseLen, 28); if (L < 20 || L > 45) L = 28;
    return { back: toISO(addDays(birth, days)), len: L + Math.floor(srand(seed + '|flen|' + ap.birth) * 5) };
  }
  ...
  function postpartumHtml(f) {
    ...
    return '<div class="pb-widget" style="--ac:' + AC + ';--hex:' + HEX + ';--soft:' + SOFT + ';">' +
      ...
      '<div style="flex:1;min-width:0;"><div style="font-size:10px;font-weight:700;color:var(--pb-t1);">' + (loss ? 'После потери · день ' : 'После родов · день ') + esc(pp) + '</div><div style="display:flex;align-items:center;gap:6px;margin-top:2px;"><span style="font-size:8px;font-weight:700;color:' + HEX + ';">' + esc(f.recovery) + '</span><span style="font-size:8px;color:var(--pb-t3);">· ' + (b < 3 ? 'выделения ещё ' + Math.max(0, total - pp) + ' дн' : (num(f.pp_back_in, 0) > 0 ? 'цикл вернётся ~через ' + f.pp_back_in + ' дн' : (loss ? 'цикл вернётся со дня на день' : 'лохии прошли'))) + '</span></div></div>' +
      ...
      '<div style="display:flex;justify-content:space-between;font-size:6.5px;margin-bottom:11px;">' + lb + '</div>' +
      '<div style="display:flex;gap:5px;margin-bottom:9px;">' + cell('🩸', 'Выделения', f.lochia, bleedColor) + cell('🌿', 'Восстан.', f.recovery, HEX) + cell('⚡', 'Энергия', f.energy, energyColorOf(f.energy)) + (loss || num(f.pp_back_in, 0) > 0 ? cell('📅', 'Цикл', num(f.pp_back_in, 0) > 0 ? 'через ' + f.pp_back_in + ' дн' : 'скоро', HEX) : cell('🍼', 'Лактация', f.lactation, HEX)) + '</div>' +
  ...
  function calBar(f, selISO) {
    if (!selISO) return '<div style="font-size:9px;color:var(--pb-t3);margin-top:8px;line-height:1.4;">тапни день, чтобы отметить события или сделать его «сегодня»</div>';
    var sd = parseISO(selISO), lbl = sd.getDate() + ' ' + MON_RU[sd.getMonth()] + ' ' + sd.getFullYear();
    var heat = f.state === 'heat' || f.slick != null, rut = f.state === 'rut' || f.knot != null;
    var preg = f.state === 'pregnancy' || f.week != null, pp = f.state === 'postpartum' || f.pp_day != null;
    function btn(act, label, mod) { return '<button class="pb-cal-act' + (mod ? ' ' + mod : '') + '" data-act="' + act + '" type="button">' + label + '</button>'; }
    var rows = '';
    function grp(title, inner) { if (inner) rows += '<div class="pb-cal-grp"><span class="pb-cal-gl">' + title + '</span><span class="pb-cal-gb">' + inner + '</span></div>'; }
    if (rut) grp('цикл', btn('rut', '🐺 начался гон'));
    else if (preg) grp('беременность', btn('due', '🍼 сюда ПДР') + btn('loss', '💔 прервалась', 'pb-soft'));
    else if (!pp) {
      grp('цикл', heat ? btn('heat', '🔥 началась течка') : btn('period', '🩸 месячные') + btn('ovu', '💜 овуляция'));
      grp('близость', btn('act0', '💞 без защиты') + btn('act1', '🛡 с защитой'));
      grp('тест', btn('test', '🧪 сделать') + btn('testp', '✅ был «+»') + btn('testn', '❌ был «−»'));
    }
    grp('дата', btn('today', '📅 это сегодня'));
    return '<div class="pb-cal-bar">' +
      '<div class="pb-cal-bh"><span class="pb-cal-bd">' + lbl + '</span><button class="pb-cal-act pb-x" data-act="cancel" type="button" title="закрыть">✕</button></div>' +
      rows + '</div>';
  }
  ...
  async function markDay(date, type) {
    ...
    else if (type === 'test' || type === 'testp' || type === 'testn') {
      var gdt = await loadGameDate();
      if (diffDays(date, gdt) > 0) { toast('тест можно отметить только сегодня или раньше'); renderCalendar(); return; }
      var rt = await loadRepro(), isoT = toISO(date);
      // «был «+»/«−»» — результат из сюжета (канон), «сделать тест» — заранее решённый кубик
      var resT = type === 'testp' ? '+' : type === 'testn' ? '-' : testOutcome(rt, isoT);
      rt.tests = rt.tests.filter(function (t) { return t.d !== isoT; }); rt.tests.push({ d: isoT, r: resT });
      diagPush('тест', isoT + ' · ' + (resT === '+' ? '«+»' : '«−»') + (type === 'test' ? ' (кубик)' : ' (отмечен вручную)'));
  ...
  async function processEvents(f, gd) {
    ...
    var tr = String(f.test || '').toLowerCase();
    var res = /полож|posit|две полоск|две полосы|\+/.test(tr) ? '+' : /отриц|negat|одна полоск|однa полоск/.test(tr) ? '-' : null;
    // модель могла ответить просто «да/сделан» — берём исход, который мы ей заранее подсказали
    if (!res && /^\s*(да|сделан|yes|true)/.test(tr)) res = testOutcome(r, iso);
    var pregnant = false;
    if (res) {
  ...
  function reproHtml(f, col) {
    ...
    if (!acts.length && !tests.length && !delay && !f._first) return '';
    ...
    if (f._first) L.push('<div style="opacity:.85;">🌱 первый цикл после восстановления — может быть нерегулярным</div>');
  ...
  async function reproInjectParts(f) {
    ...
    var D = num(f.delay, 0);
    if (f._first) o.facts.push('цикл только вернулся после беременности — первый цикл нерегулярный, тело ещё привыкает');
  ...
  async function computeLock(gd) {
    ...
    if (a && a.kind === 'postpartum' && a.birth) {
      var rl2 = reproOn() ? await loadRepro() : null, bk = ppReturn(a, (rl2 && rl2.seed) || 'x', 28);
      if (diffDays(gd, parseISO(bk.back)) < 0) return { kind: 'postpartum', pp_day: Math.max(1, diffDays(gd, parseISO(a.birth)) + 1), pp_total: num(a.total, LOCHIA_LEN), pp_loss: a.loss || '', back: bk.back };
      return { kind: cycleKind() === 'heat' ? 'heat' : 'cycle', day: Math.min(diffDays(gd, parseISO(bk.back)) + 1, bk.len), length: bk.len, delay: 0, first: 1 };
    }
    var kind = cycleKind();
    if (a && a.kind === kind && a.start) { var rr = reproOn() ? await loadRepro() : null; var cs = cycleState(a, rr, gd); return { kind: kind, day: cs.day, length: cs.len, delay: cs.delay, first: a.firstAfter ? 1 : 0 }; }
    return null;
  }
  async function applyAnchor(f, gd) {
    ...
    if (f.state === 'postpartum' || f.pp_day != null) {
      ...
      var rpp = reproOn() ? await loadRepro() : null, seedP = (rpp && rpp.seed) || 'x';
      var back = ppReturn(ap, seedP, f.length);
      if (diffDays(gd, parseISO(back.back)) >= 0) {
        // цикл вернулся: первый цикл идёт от даты возврата и бывает чуть длиннее
        var kindBack = cycleKind() === 'heat' ? 'heat' : 'cycle';
        await tavo.set(ANCHOR_KEY, { kind: kindBack, start: back.back, length: back.len, firstAfter: true }, 'chat');
        ['pp_day', 'pp_total', 'pp_loss', 'pp_week', 'lochia', 'bleeding_pct', 'recovery', 'lactation'].forEach(function (k) { delete f[k]; });
        f.state = kindBack; // дальше отработает обычная ветка цикла
      } else {
        ...
        f.pp_back = back.back; f.pp_back_in = Math.max(0, diffDays(parseISO(back.back), gd));
        return f;
      }
    }
    var kind = cycleKind();
    ...
    if (an && an.kind === kind && an.start) {
      ...
      if (cs.rolled) { an.start = toISO(cs.start); delete an.firstAfter; await tavo.set(ANCHOR_KEY, an, 'chat'); }
      if (an.firstAfter) f._first = 1;
